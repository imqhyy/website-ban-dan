...
if (manageButtons && brandModal) {
    ...
    document.querySelector('.close-button-for-edit-brands').onclick = () => brandModal.style.display = "none";
...
function fetchBrands(catId) {
    fetch(`forms/quanlyloaisanpham/ajax_handle_brands.php?action=fetch&cat_id=${catId}`)
...
function deleteBrand(brandId, catId) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch(`forms/quanlyloaisanpham/ajax_handle_brands.php?action=delete&brand_id=${brandId}&cat_id=${catId}`)
                .then(res => res.text())
                .then(data => {
...
function deleteBrandGlobal(id, name) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch(`forms/quanlyloaisanpham/ajax_handle_brands.php?action=delete_global&id=${id}`)
                .then(res => res.text())
                .then(data => {
                    if (data.trim() === 'success') {
                        fetchGlobalBrands(); // Cập nhật lại bảng tổng
                        // Nếu đang mở Modal của phân loại nào đó, ta cũng nên load lại Modal đó
                        const currentCatId = document.getElementById('current-category-id').value;
...
function openEditBrandModal(brandId) {
    fetch(`forms/quanlyloaisanpham/ajax_handle_brands.php?action=get_brand_detail&id=${brandId}`)
        .then(res => res.json())
        .then(data => {
...
// Xử lý đóng Modal
document.querySelector('.close-button-for-edit-info-brand').onclick = function() {
    editInfoBrandModal.style.display = 'none';
};
...
function addNewBrandGlobalDirect() {
    ...
    fetch('forms/quanlyloaisanpham/ajax_handle_brands.php', { method: 'POST', body: formData })
        .then(res => res.text())
        .then(data => {
            if (data.trim() === 'success') {
                nameInput.value = '';
...
if (editButtons) {
    editButtons.forEach(button => {
        button.addEventListener('click', function (event) {
            ...
            const closeBtn = document.querySelector('.close-button-for-edit-info-category');
            if (closeBtn) {
