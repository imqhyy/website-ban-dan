...
if ($action === 'add_product') {
    try {
        ...
        $stmt->execute([
            ...
            $_POST['profit_margin'] ?? 0, $_POST['discount_percent'] ?? 0, $acc_json,
            $_POST['highlight_1_title'] ?? '', $_POST['highlight_1_content'] ?? '', 
            $_POST['highlight_2_title'] ?? '', $_POST['highlight_2_content'] ?? '', 
            $_POST['highlight_3_title'] ?? '', $_POST['highlight_3_content'] ?? '', 
            $_POST['highlight_4_title'] ?? '', $_POST['highlight_4_content'] ?? ''
        ]);
...
if ($action === 'fetch_list') {
    ...
    if (!empty($_GET['product_type'])) $conditions[] = "p.product_type = " . $pdo->quote($_GET['product_type']);
    if (!empty($_GET['brand_id'])) $conditions[] = "p.brand_id = " . (int)$_GET['brand_id'];
...
if ($action === 'update_product') {
    try {
        ...
        if ($old_dir !== $new_dir) {
            ...
            if (file_exists($old_dir) && count(scandir($old_dir)) == 2) rmdir($old_dir);
        } else {
        ...
        $pdo->prepare($sql)->execute([
            $p_name, $p_type, $brand_id, $final_images_list,
            $_POST['summary_description'], $_POST['detailed_overview'], $_POST['profit_margin'], $_POST['discount_percent'], $acc_json,
            $_POST['highlight_1_title'], $_POST['highlight_1_content'], $_POST['highlight_2_title'], $_POST['highlight_2_content'], 
            $_POST['highlight_3_title'], $_POST['highlight_3_content'], $_POST['highlight_4_title'], $_POST['highlight_4_content'],
            $p_id
        ]);
