...
if (empty($adminUsername)) { header('Location: admin_login.php'); exit; }

$action = $_POST['action'] ?? '';
if ($action && !empty($_POST['review_id'])) {
    $rid = (int)$_POST['review_id'];
    if ($action === 'hide')   $pdo->prepare("UPDATE reviews SET status='hidden'  WHERE id=?")->execute([$rid]);
    if ($action === 'show')   $pdo->prepare("UPDATE reviews SET status='visible' WHERE id=?")->execute([$rid]);
    if ($action === 'delete') $pdo->prepare("DELETE FROM reviews WHERE id=?")->execute([$rid]);
              ...
              <table class="table table-bordered table-hover align-middle">
                <thead class="table-light">
                  <tr>
                    ...
                    <th>Sao</th><th>Nội dung</th><th>Ngày</th>
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td style="max-width:300px; white-space:normal; overflow-wrap: break-word;">
                      <div style="margin-bottom: 8px; font-size: 14px;"><?= nl2br(htmlspecialchars($rv['comment'])) ?></div>
                        ...
                        <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                          <?php foreach (explode(',', $rv['image_path']) as $img): ?>
                            <a href="../<?= htmlspecialchars(trim($img)) ?>" target="_blank">
                              <img src="../<?= htmlspecialchars(trim($img)) ?>" style="width: 48px; height: 48px; object-fit: cover; border-radius: 4px; border: 1px solid #ccc; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.1)'" onmouseout="this.style.transform='scale(1)'"/>
                            </a>
                          <?php endforeach; ?>
                        </div>
                      <?php endif; ?>
                      <?php if ($rv['is_purchased']): ?>
                        <div style="margin-top: 8px; font-size: 11px; color: #198754;"><i class="bi bi-check-circle-fill"></i> Đã mua hàng</div>
                      <?php endif; ?>
                    </td>
                  ...
                  <tr><td colspan="8" class="text-center text-muted">Chưa có đánh giá nào.</td></tr>
                  <?php endif; ?>
                </tbody>
              </table>
            </div>
